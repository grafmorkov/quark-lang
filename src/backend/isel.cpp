...
void ISel::emit_call(const IRProgram& program, const IRFunction& fn, const IRCall& x) {
    ...
    for (std::size_t i = n; i > max_reg; --i) {
        ...
        text.mov_mem_r64(x86::mem_base(x86::RSP, static_cast<int64_t>(shadow + slot * 8)), x86::RAX);
    }
    ...
    const uint32_t callee_sym = symbol_index.count(abi_name(callee))
        ? symbol_index[abi_name(callee)]
        : add_runtime_symbol(abi_name(callee));
    (void)callee_sym;
    text.call_rel32(abi_name(callee));
            ...
            if (x.target_kind == ast::TypeKind::String) {
                if (is_integer(x.src_kind)) {
                    ...
                    if (is_signed_int(x.src_kind)) {
                        if (src_sz == 4)      text.movsxd_r64_mem32(x86::RDI, src);
                        else if (src_sz == 2) text.movsx_r64_mem16(x86::RDI, src);
                        else if (src_sz == 1) text.movsx_r64_mem8(x86::RDI, src);
                        else                  text.mov_r64_mem(x86::RDI, src);
                    } else {
                        if (src_sz == 4)      text.mov_r32_mem(x86::RDI, src);
                        else if (src_sz == 2) text.movzx_r32_mem16(x86::RDI, src);
                        else if (src_sz == 1) text.movzx_r32_mem8(x86::RDI, src);
                        else                  text.mov_r64_mem(x86::RDI, src);
                    }
                    if (is_signed_int(x.src_kind)) {
                    ...
                    text.mov_r64_mem(x86::RDI, dst);
                    text.call_rel32("qk_format_f64");
                } else {
                    text.mov_r64_mem(x86::RDI, src);
                    text.call_rel32("qk_format_f64");
                }
                text.mov_mem_r64(dst, x86::RAX);
                return;
